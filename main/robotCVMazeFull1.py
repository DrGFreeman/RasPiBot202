...
def forward(speed, time):
    ...
    while timer.isWithin(time):
        rpb202.motionCtrl.setSpeed(speed, 0)
...
##    rpb202.motionCtrl.forwardDist(speed, dRead, stop = False)
    print "Intersection"
    rpb202.motionCtrl.forwardDist(speed, 40, stop = False)
    intersection = lineTracker.getIntersection()
    print intersection

    ##  Move to center of intersection
##    rpb202.motionCtrl.forwardDist(speed - 70, dInt, decel = True)
    rpb202.motionCtrl.forwardDist(speed, 220, decel = True)
##    rpb202.motionCtrl.stop()
